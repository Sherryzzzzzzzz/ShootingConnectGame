...
namespace ShootingGame.Shared.Protocol
{
    ...
    public enum ActionCode
    {
        ...
        PlayerLeft = 209,
        Disconnect = 210,
        RpcCall = 211,          // RPC 调用（新框架）
        DeltaState = 212,       // I/P帧状态同步（新框架）
        HeroSelected = 213,     // 选角（新框架）
        HeroConfirmed = 214,    // 选角锁定（新框架）
    }
    ...
    public class MainPack
    {
        ...
        public string Str;
        public int IntVal;
        public byte[] RpcPayload;  // RPC 二进制负载（新框架）
    ...
    public class PlayerOperation
    {
        ...
        public float PosX;   // 客户端预测位置X（避免服务端重复模拟）
        public float PosY;
        public float PosZ;
        public float VelX;   // 客户端速度
        public float VelZ;
    ...
    public static class ProtoSerializer
    {
        ...
        public static void WriteMainPack(PacketWriter w, MainPack pack)
        {
            ...
            {
                w.WriteByte(1);
                w.WriteInt32(pack.RpcPayload.Length);
                w.WriteBytes(pack.RpcPayload, 0, pack.RpcPayload.Length);
            }
            else w.WriteByte(0);
        ...
        public static MainPack ReadMainPack(PacketReader r)
        {
            ...
            if (r.ReadByte() == 1)
            {
                int rpcLen = r.ReadInt32();
                pack.RpcPayload = r.ReadBytes(rpcLen);
            }
        ...
        private static void WritePlayerOperation(PacketWriter w, PlayerOperation op)
        {
            ...
            // 客户端预测位置（避免服务端重复模拟导致不同步）
            w.WriteFloat(op.PosX);
            w.WriteFloat(op.PosY);
            w.WriteFloat(op.PosZ);
            w.WriteFloat(op.VelX);
            w.WriteFloat(op.VelZ);
            w.WriteBool(op.IsGrounded);
        }
